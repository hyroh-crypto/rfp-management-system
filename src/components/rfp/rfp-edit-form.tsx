...
import { useUpdateRfp } from '@/hooks/use-rfps'
...
export function RfpEditForm({ rfp }: RfpEditFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      received_date: rfp.received_date.split('T')[0],
      due_date: rfp.due_date.split('T')[0],
      estimated_budget: rfp.estimated_budget || '',
      estimated_duration: rfp.estimated_duration || '',
      assignee_id: rfp.assignee_id || '',
    },
  })
  ...
  const onSubmit = async (data: RfpFormData) => {
    try {
      ...
      await updateRFP.mutateAsync({ id: rfp.id, data: payload })
      router.push(`/rfps/${rfp.id}`)
    } catch (error) {
      console.error('Failed to update RFP:', error)
      alert('RFP 수정 중 오류가 발생했습니다.')
    }
  }
